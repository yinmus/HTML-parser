...
def ask(ext, def_val=None):
    if def_val == "None": return None
    if def_val: return def_val
    files = f(ext)
    if files:
        for i, x in enumerate(files, 1): print(f"{i}. {x}")
    else: print(f"No {ext} files.")
    
    ch = input("Number or path: ").strip()
...
def kill(p):
    if sys.platform.startswith("linux"):
        ...
        os.system(f"netstat -ano | findstr :{p} > temp.txt && for /f \"tokens=5\" %i in (temp.txt) do taskkill /F /PID %i && del temp.txt")
    else:
        print("Kill manually.")
...
if port_in_use(port):
    if input(f"Port {port} in use. Kill? [y/n]: ").strip().lower() == "y":
        kill(port)
    else:
        sys.exit(1)
    ...
    def do_GET(self):
        if self.path == '/' and HTML:
            with open(HTML, "r") as f:
                lib.parse_html(f.read().encode('utf-8'))
        if self.path == '/styles.css' and CSS:
            with open(CSS, "r") as f:
                lib.parse_css(f.read().encode('utf-8'))
        if self.path == '/script.js' and JS:
            with open(JS, "r") as f:
                lib.parse_js(f.read().encode('utf-8'))
        return super().do_GET()
